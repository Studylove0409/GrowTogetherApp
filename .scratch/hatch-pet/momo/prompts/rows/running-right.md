Create a single horizontal sprite strip for the Codex app digital pet `momo` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Small chibi digital pet inspired by a gentle Chinese Flutter developer building GrowTogetherApp, a warm couples growth app. Cream-pink body, peach cheeks, tiny leaf-sprout ears, subtle Flutter-blue scarf accent, kind focused expression, cozy growth-companion personality..
